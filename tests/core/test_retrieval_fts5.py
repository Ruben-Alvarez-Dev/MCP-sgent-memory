...
async def test_fts5_search_finds_content(tmp_path):
    ...
    await db.upsert("p1", None, {"content": "JWT authentication middleware for user sessions", "agent_scope": "shared", "layer": 1})
...
async def test_synonym_expansion_produces_terms(tmp_path):
    ...
    await db.ensure_collection()
    await db.upsert("p1", None, {"content": "JWT authentication middleware", "agent_scope": "shared", "layer": 1})
...
async def test_entity_boost_changes_ranking(tmp_path):
    ...
    await db.ensure_collection()
    await db.upsert("p1", None, {"content": "AuthService implements JWT authentication", "agent_scope": "shared", "layer": 1})
    await db.upsert("p2", None, {"content": "database connection pooling", "agent_scope": "shared", "layer": 1})
...
async def test_zero_embedding_dependency(tmp_path):
    ...
    await db.ensure_collection()
    await db.upsert("p1", None, {"content": "how to implement JWT authentication", "agent_scope": "shared", "layer": 1})
    results = await db.search_fts("JWT authentication", limit=5, filter={"must": [{"key": "agent_scope", "match": {"value": "shared"}}]})
    assert len(results) >= 1
    assert results[0]["id"] == "p1"
...
async def test_entity_boost_clamped_to_one(tmp_path):
    ...
    await db.ensure_collection()
    await db.upsert("p1", None, {"content": "AuthService JWT OAuth SSO MFA authentication authorization", "agent_scope": "shared", "layer": 1})
    results = await db.search_fts("AuthService", limit=5, filter={"must": [{"key": "agent_scope", "match": {"value": "shared"}}]})
    assert len(results) >= 1
