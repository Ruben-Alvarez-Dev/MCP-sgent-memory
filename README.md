...
```
MCP-agent-memory/
├── config/                       # mcp.json (generated), .env (gitignored)
...
│       │   └── entities/
│       └── .system/             # counter.json
├── openspec/                     # Specs + gated milestones M0–M9 (storage, identity, trunk…)
├── scripts/                      # register_agent.py, launch-unified.sh, generate-mcp-config.sh,
│                                 #   run_eval.py, lifecycle.sh, migrate_to_memory_db.py
├── logs/
├── src/
...
# it goes to the macOS Keychain (see Identity & Isolation below).
MEMORY_AGENT_ID=pi-agent
```

### MCP Client Configuration

Generated by `scripts/generate-mcp-config.sh` (install to `~/.pi/mcp.json` with
`--install`). When `MEMORY_AGENT_ID` is set, it emits the **Keychain-backed
launcher** — strict mode with no secrets in any file:

```json
{
  "mcpServers": {
    "MCP-agent-memory": {
      "command": "/path/to/scripts/launch-unified.sh",
      "args": [],
      "env": {
...
Without `MEMORY_AGENT_ID`, the generator emits the plain python command and the
server boots in `open` mode (visible WARN).

---

## Identity & Isolation

Each MCP server instance runs under a **harness-asserted identity** (M4).
Strict-mode setup (three commands, no secrets in files):

```bash
# 1. Register — prints the token ONCE
python scripts/register_agent.py register pi-agent

# 2. Store it in the macOS Keychain (never in mcp.json / .env / shell rc)
security add-generic-password -s "memory-zero/pi-agent" -a "$USER" -w '<token>'

# 3. Set MEMORY_AGENT_ID=pi-agent in config/.env and regenerate the client config
scripts/generate-mcp-config.sh --install
```

`scripts/launch-unified.sh` (referenced by the generated config) pulls the token
