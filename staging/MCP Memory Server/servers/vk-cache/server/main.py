...
async def request_context(
    ...
    """LLM requests context. Returns a ContextPack.

    This is the PULL side of the bidirectional protocol.
    Call when the LLM needs relevant context.
    ...
    """
    allowed_scopes = [s.strip() for s in scopes.split(",") if s.strip()] if scopes else []

    # Query all sources in parallel
    qdrant_task = asyncio.create_task(search_qdrant(query, limit=MAX_ITEMS))
    engram_task = asyncio.create_task(search_engram(query, limit=5))

    qdrant_results, engram_results = await asyncio.gather(
        qdrant_task, engram_task, return_exceptions=True
    )
    ...
    if isinstance(engram_results, list):
        results.extend(engram_results)
    ...
    if allowed_scopes:
        ...
        for r in results:
            r_scope = r.get("scope", "")
            if any(s in r_scope for s in allowed_scopes):
                filtered.append(r)
        results = filtered

    # Compress
    ranked = await _compress_results(results, MAX_ITEMS)
    ...
    return json.dumps({
        ...
        "injection_text": pack.to_injection_text(),
        "metadata": {
            "total_queried": len(results),
            "total_returned": len(ranked),
            "token_estimate": token_est,
            "within_budget": token_est <= token_budget,
        },
    }, indent=2)
