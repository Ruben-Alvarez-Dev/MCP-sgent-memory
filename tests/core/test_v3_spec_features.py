...
def test_pruner_keeps_python_structure_with_ast_grade_collapse():
    ...
    assert "def method" in pruned or "..." in pruned  # collapsed is OK
...
def test_sequential_thinking_stages_and_applies_change_sets(tmp_path: Path, monkeypatch):
    module_path = ROOT / "src" / "sequential_thinking" / "server" / "main.py"
...
@pytest.mark.skip(reason="Proximity ranking by entities not yet implemented in _rank_and_fuse")
def test_repo_proximity_prioritizes_structurally_closer_items():
    ...
    close_item = retrieval.ContextItem(
        ...
        source_name="qdrant",
        score=0.8,
        metadata={"path": "pkg/main.py", "dependencies": ["pkg.dep"], "entities": ["helper"]},
    )
    far_item = retrieval.ContextItem(
        ...
        source_name="qdrant",
        score=0.85,
        metadata={"path": "docs/notes.py", "dependencies": [], "entities": ["other"]},
    )
