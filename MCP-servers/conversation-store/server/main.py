...
async def save_conversation(
    ...
    Args:
        ...
        messages: JSON array of messages [{"role": "user", "content": "..."}, ...]
        messages_json: Legacy alias for messages (deprecated, use messages).
        metadata: Optional JSON metadata.
    """
    await ensure_collection()

    raw = messages or messages_json
    messages_list = json.loads(raw)
    meta = json.loads(metadata) if metadata else {}
    text_content = json.dumps(messages_list, ensure_ascii=False)
